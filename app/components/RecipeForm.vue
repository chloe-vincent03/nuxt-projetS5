...
const props = defineProps<{
  initialData?: FullRecipe | null
  isEditMode?: boolean
}>()

// Defining emits
const emit = defineEmits<{
  (e: 'submit', payload: RecipePayload): void
  (e: 'cancel'): void
}>()
...
onMounted(async () => {
  ...
  if (props.initialData) {
    ...
    cuisine_id.value = data.cuisine_id
    goal_id.value = data.goal_id
    DietaryInformation_id.value = data.DietaryInformation_id
    AllergiesInformation_id.value = data.AllergiesInformation_id
    ...
    if (data.instructions && data.instructions.length > 0) {
      instructions.value = data.instructions.map(i => ({
  ...
  <div class="recipe-form">
    ...
    <form @submit.prevent="onSubmit">
      ...
      <div class="recipe-form__section">
        <div class="recipe-form__section-list">
          <div v-for="(ingredient, index) in ingredients" :key="index" class="recipe-form__section-item">
            ...
            <MButton size="tiny" variant="supp" type="button" @click="removeIngredient(index)">Supprimer</MButton>
          </div>
        </div>
        <MButton size="small" variant="outline" type="button" class="recipe-form__section-add" @click="addIngredient">Ajouter un ingrédient</MButton>
      </div>
      ...
      <div class="recipe-form__section">
        <div class="recipe-form__section-list">
          <div v-for="(step, index) in instructions" :key="index" class="recipe-form__section-item">
            ...
            <MButton size="tiny" variant="supp" type="button" @click="removeInstruction(index)">Supprimer</MButton>
          </div>
        </div>
        <MButton size="small" variant="outline" type="button" class="recipe-form__section-add" @click="addInstruction">Ajouter une étape</MButton>
      </div>
      ...
      <div class="recipe-form__submit">
        ...
        <MButton v-if="isEditMode" variant="outline" type="button" @click="$emit('cancel')">
          Annuler
        </MButton>
      </div>
    </form>
